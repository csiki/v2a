function [result, thinnedOrientation] = thinning(matrix, oriensMatrix, oriensMatrixIndex, method)
% VERSION 27/02/05
...
matrixB = zeros(h+2, w+2);
oriensMatrixB = zeros(h+2, w+2);
matrixB(h+2,w+2) = 0;
matrixB(2:h+1,2:w+1) = matrix(:,:);
oriensMatrixB(h+2,w+2) = 0;
oriensMatrixB(2:h+1,2:w+1) = oriensMatrix(:,:);

oriensMatrixIndexB = zeros(h+2, w+2);
oriensMatrixIndexB(h+2, w+2) = 0;
oriensMatrixIndexB(2:h+1,2:w+1) = oriensMatrixIndex;
...
if (method == 1) % simple thinning
  ...
  for I=2:h+1 % the rows
    for J=2:w+1 % the columns
      ...
      thinnedOrientation(I,J) = ( (matrixB(I,J) >= pnt1) & (matrixB(I,J) >= pnt2) ) * oriensMatrixIndexB(I,J);
    end
...
thinnedOrientation = thinnedOrientation(2:h+1, 2:w+1);