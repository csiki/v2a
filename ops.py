...
class batch_norm(object):  # FIXME depricated, not used
...
def conv2d(x, inputFeatures, outputFeatures, name):  # FIXME depricated, not used
    with tf.variable_scope(name):
...
def conv_transpose(x, outputShape, name):  # FIXME depricated, not used
    with tf.variable_scope(name):
        # h, w, out, in
...
# leaky reLu unit
def lrelu(x, leak=0.2, name="lrelu"):  # FIXME depricated, not used
    with tf.variable_scope(name):
        f1 = 0.5 * (1 + leak)
        f2 = 0.5 * (1 - leak)
...
def dense(x, input_features, output_features, scope=None, with_w=False):  # FIXME used, but can be swapped to tf implementation
...
def merge(images, size):  # FIXME depricated, not used
